...
def index(request):
    return render('polls/index.html', {}, request)
...
def question(request, slug):
    ...
    if request.method == 'GET':
        ...
        return render('polls/question.html', payload, request)
...
def results(request, slug):
    ...
    return render('polls/results.html', payload, request)
        
    

def create(request):
    if request.method == 'POST':    
        form = forms.CreatePoll(request, request.POST)
        if form.is_valid():
            question = form.save()
            try:
        ...
        form = forms.CreatePoll(request)
    payload = {'form':form}
    return render('polls/create.html', payload, request)

def help(request):
    payload = {}
    return render('polls/help.html', payload, request)
...
def render(template_name, payload, request):
	...
	if request.user.is_authenticated():
		your_polls = Question.objects.filter(user=request.user)
	else:
		your_polls = 0
